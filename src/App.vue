...
function objectificationSpecialTime(json, timezone) {
  ...
  for (const meet in json) {
    ...
    test.numDay = dayNumber(test.day);
    test.startTime = json[meet].time.split(" - ")[0];
    test.endTime = json[meet].time.split(" - ")[1];
    test.app = json[meet].app;
    test.title = json[meet].title;
    test.link = json[meet].link;
    test = convertToLocal(
      test,
      timezone,
      json[meet].year,
    ...
    if (typeof meets[test.startTime + "- " + test.endTime] === "undefined") {
      let dayObject = {};
      dayObject.title = test.title;
      dayObject.app = test.app;
      dayObject.link = test.link;
      dayObject.date = json[meet].date;
      dayObject.month = json[meet].month;
      dayObject.year = json[meet].year;

      meets[test.startTime + "- " + test.endTime] = {};
      meets[test.startTime + "- " + test.endTime][test.day] = dayObject;
    } else {
      let dayObject = {};
      dayObject.title = test.title;
      dayObject.app = test.app;
...
function objectification(json, timezone) {
  ...
  for (const day in json) {
    for (const time in json[day]) {
      let test = new Object();
      test.day = day;
      test.numDay = dayNumber(day);
      test.startTime = time.split(" - ")[0];
      test.endTime = time.split(" - ")[1];
      test.title = json[day][time].title;
      test.app = json[day][time].app;
      test.link = json[day][time].link;
      test = convertToLocal(test, timezone);
  ...
  watch: {
    timeZoneSelect() {
      axios
        ...
        .then(
          (response) =>
            (this.meetingtimelist = sortOnKeys(
      ...
      axios
        .get("") // set url special json
        .then(
          (response) =>
            (this.meetingSpecial = sortOnKeys(
